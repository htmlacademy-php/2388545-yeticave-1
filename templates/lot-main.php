        ...
        <div class="lot-item__right">
            <div class="lot-item__state">
                <div class="lot-item__timer timer <?= htmlspecialchars($timer_class) ?>">
                    <?php if ((int)($time_left[0] ?? 0) < 0) : ?>
                        00 : 00
                    <?php else : ?>
                        <?= htmlspecialchars($formatted_time_left[0] ?? '00') . " : " . htmlspecialchars($formatted_time_left[1] ?? '00') ?>
                    <?php endif; ?>
                </div>
                    ...
                    <?php else : ?>
                        <?php foreach ($rate_history as $rate) : ?>
                            <tr class="history__item">
                                <td class="history__name"><?= htmlspecialchars($rate['login'] ?? 'Без имени') ?></td>
